...
?>
    ...
    <div class="mt-3">
        	...
        	<br><h6>On successfull connection, you can post someone links, messages and get Rewards(credits).</h6>
        <hr><?php die;?>
    ...
    <?php
		...
		$url=@$linked_post['url'];
		$user=@$linked_post['uid'];
		$cpc=@$linked_post['cpc'];
		$post=@$linked_post['post_id'];
    	
		
		$msg="traffexchange.in";
	?>
    
    <h2>Linkedin Post</h2>
    <p>When You Click <strong>"Post on Linkedin"</strong> Button it Automatically Post On Your Linkedin Account</p>
    
    
    <button id="user_linked">Post on Linedin</button>     
     
<script>
	$(document).ready(function(e) {
        $('#user_linked').click(function(){
			$.ajax({
				type:'POST',
				url:"<?php echo site_url('Linkedin_data/user_post'); ?>",
				data:{
					msg:"<?php echo $msg; ?>"
				},
